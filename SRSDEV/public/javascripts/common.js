...
$("#ajaxform").submit(function(e)
		...
		// console.log("Start Date : "+postData[0].value);
		// console.log("End Date : "+postData[1].value);
		// console.log("Store : "+postData[2].value);
		 //console.log("Type : "+postData[3].value);
		 //console.log("Sort: "+postData[4].value);
		$.ajax(
    	{
 ...
 $('#tab1').addClass("active");
  ...
  function monthlyReportsUpdateList(element) {
  	 var file = element;
		var flag =true;
		...
		var MRFileType = fileExt[fileExtLength-1];
		//console.log("fileName is : "+fileName);
		//console.log("file type is : "+MRFileType);
		var monthlyReportType=document.getElementById("monthlyReportType").value;
			if(monthlyReportType == "Sira Reports"){
				if(MRFileType == 'xls' || MRFileType == 'xlsx' || MRFileType == 'XLS' || MRFileType == 'XLSX'){
				...
				if(MRFileType == 'pdf' || MRFileType == 'jpeg'){
		...
		var output = document.getElementById('fileList');
			if(monthlyReportType == "Sira Reports"){
				alert("only xls and xlsx file types allowed");
				element.value="";
  	 ...
  	 function HoFormUpdateList(element) {
		...
		console.log("file type is : "+HoFileType);
			...
			if(HoFileType == 'pdf' || HoFileType == 'docx' || HoFileType == 'doc' || HoFileType == 'xlsx' || HoFileType == 'xls' || HoFileType == 'JPEG' || HoFileType == 'PDF'){
		...
		$(".fancyboxPDF").fancybox({
			...
			beforeShow: function () {
	            if (this.title) {
	                ...
	                // Add Form to Title
	                this.title += '<form action="/SRSMOBILE/InvoiceInventory/list/move/'+invoiceId+'" method="post"><div class="table-responsive"><table style="background-color: white;border-color: #fff;" class="table table-bordered fonSiz"><tr><td><div class="clearfix" id="category_field"><label for="category" id="pageTextFontSize">Invoice Category</label><div class="input"><select id="invoiceCategoryAtFancyBox" name="category" onchange="disableSupplierListAtFancyBox()" required><option class="blank" value="">-- Choose --</option><option value="Fuel">Fuel</option><option value="Others">Others</option></select> <span class="help-inline"></span></div></div></td><td><div class="clearfix" id="invoiceDate_field"><label for="invoiceDate" id="pageTextFontSize">Invoice Date</label><div class="input"><input type="text" class="datepickerFancyBox textBoxesStyles" name="invoiceDate" required><span class="help-inline"></span></div></div></td><td><div class="clearfix" id="paymentTerms_field"><label for="paymentTerms" id="pageTextFontSize">Payment Terms</label><div class="input"><select id="paymentTermsFancyBox" name="paymentTerms" required><option class="blank" value="">-- Choose --</option><option value="CashPaid">CashPaid</option><option value="DD">DD</option><option value="FortNightly">FortNightly</option><option value="Weekly">Weekly</option><option value="Monthly">Monthly</option></select> <span class="help-inline"></span></div></div></td><td><div class="clearfix" id="supplier_field"><label for="supplier" id="pageTextFontSize">Supplier</label><div class="input"><select id="supplierListAtFancyBox" name="supplier" required onfocus="selectBoxOptionsInFancyBox()"><option class="blank" value="">-- Choose a Supplier--</option></select> <span class="help-inline"></span></div></div></td><td><input type="submit" value="Done" class="btn btn-success btnStylesInProcess">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="/SRSMOBILE/InvoiceInventory/delete/'+invoiceId+'" class="btn btn-danger btnStylesInProcess" onclick="return confirm('+" 'Are You sure want to Delete ?' "+')">Delete</a></td></tr></table></div></form>';
  ...
  function addSuppDetails(payoutType,strKey)
    ...
    success : function(data) {
      val = $('#suppliersid option:selected').text(); 
  	  res = val.split("-");
    //	alert(data);
     newRowContent = "<tr><td>"+res[0]+" </td><td>"+res[1]+"</td><td><input type=\"text\" name=\"" +data + "_Amt\"  onblur=\"payAmtNullCheck(this)\" id=\"" +data + "_Amt\" value=\"0.00\"></td><td><select name=\"" +data + "_Exp\" id=\"" +data + "_Exp\"></select> </td><td><input type=\"file\" name=\"" +data + "_File\" id=\"" +data + "_File\" onchange=\"safePayoutFileValidate(this)\"></td><td><textarea name=\"" +data + "_reason\" id=\"" +data + "_reason\" cols=\"30\" rows=\"4\"></textarea></td><td><INPUT TYPE=\"Button\" CLASS=\"btn btn-primary delButtonStylesInShift\" onClick=\"delRow('"+data+"')\" VALUE=\"Delete Row\"></td></tr>";
  ...
  function similarSupplierChange()
   ...
   if (validate(supp_data,$("input[name='supplierslist[]']"))){
	 ...
	 function similarSupplierChange1(storeId)
	{
       ...
       var  val = $('#similarSuppliersid option:selected').text();
       val =val.split("-");
       console.log("supp_data : "+supp_data);
    ...
    success : function(data) {
     ...
     var newRowContent = "<tr><td>"+res[1]+"</td><td>"+res[2]+"</td><td><select name=\"" +res[0]+ "_paymentTerms\" id=\"" +res[0]+ "_paymentTerms\" ></select></td><td><select name=\"" +res[0]+ "_paymentMode\" id=\"" +res[0]+ "_paymentMode\"></select></td><td><INPUT TYPE=\"Button\" id=\"b_"+supp_data+"\" CLASS=\"btn btn-primary delButtonStylesInShift\" onClick=\"delSupplierMapping('"+res[0]+"')\" VALUE=\"Delete\"></td></tr>";
...
//console.log("inside validateForSuppliers method");
 	//alert("Inside validateForSuppliers function" ); 
 	id="b_"+id;
						...
						if(data > 0){ // Ajax call returns count of Existing Timesheet for the same Time range for this employee
							...
							myJsRoutes.controllers.Application.getTimeSheetSave(emp,date,startTimeHour,startTimeMins,endTimeHour,endTimeMins,duration,textArea,leaveType).ajax({
								...
								//console.log("start time is : "+data.startTimeHour);
									if(leaveType == "None"){
										newRowContent = "<tr><td><input type=\"text\" name=\"" +rowNum + "_StartTimeHours\"  CLASS=\"smalltextBoxesStyles\" id=\"" +rowNum + "_StartTimeHours\" value=\""+data.startTimeHour+"\" readonly='readonly'></td><td><input type=\"text\" name=\"" +rowNum + "_StartTimeMins\" CLASS=\"smalltextBoxesStyles\"  id=\"" +rowNum + "_StartTimeMins\" value=\""+data.startTimeMins+"\" readonly='readonly'></td><td><input type=\"text\" name=\"" +rowNum + "_EndTimeHours\" CLASS=\"smalltextBoxesStyles\"  id=\"" +rowNum + "_EndTimeHours\" value=\""+data.endTimeHour+"\" readonly='readonly'></td><td><input type=\"text\" name=\"" +rowNum + "_EndTimeMins\"  CLASS=\"smalltextBoxesStyles\" id=\"" +rowNum + "_EndTimeMins\" value=\""+data.endTimeMins+"\" readonly='readonly'></td><td><input type=\"text\" name=\"" +rowNum + "_Duration\" CLASS=\"smalltextBoxesStyles\"  id=\"" +rowNum + "_Duration\" value=\""+data.duration+"\" readonly='readonly'></td><td><textarea name=\"" +rowNum + "_Activity\"   id=\"" +rowNum + "_Activity\" value=\""+data.activity+"\" readonly='readonly'>"+data.activity+"</textarea></td><td><INPUT TYPE=\"Button\" CLASS=\"btn btn-primary delButtonStyles\" onClick=\"delHeadOfficeTimeSheetRow('"+data+"')\" VALUE=\"Delete Row\"></td></tr>";
										$("#headOfficeTimeSheetTable tbody").append(newRowContent);
										//newRowContent = "<tr><td>"+data.startTimeHour+"</td><td>"+data.startTimeMins+"</td><td>"+data.endTimeHour+"</td><td>"+data.endTimeMins+"</td><td>"+data.duration+"</td><td>"+data.activity+"</td><td><INPUT TYPE=\"Button\" CLASS=\"btn btn-primary delButtonStylesInShift\" onClick=\"delHeadOfficeTimeSheetRow('"+data+"')\" VALUE=\"Delete Row\"></td></tr>";
										rowNum++;
...
function monthlyReportsFormValidation(){

	var monthInMonthlyReports=document.getElementById("monthlyid").value;
	var monthlyReportType=document.getElementById("monthlyReportType").value;
	
    var flag= false;            	
	if(monthInMonthlyReports == ""){
		$('#monthInMR').css('display','block');
		$("#monthInMR").css("color", "red");
		$('#monthTypeInMR').css('display','none');
		$('#fileInMR').css('display','none');
	}
	else{
		$('#monthInMR').css('display','none');
		if( monthlyReportType == ""){
			$('#monthTypeInMR').css('display','block');
			$("#monthTypeInMR").css("color", "red");
			$('#monthInMR').css('display','none');
			$('#fileInMR').css('display','none');
		}
		else{
			...
			if(fileName == ""){
				$('#monthInMR').css('display','none');
				$('#monthTypeInMR').css('display','none');
				$('#fileInMR').css('display','block');
				$("#fileInMR").css("color", "red");
			}
			else{
				$('#fileInMR').css('display','none');
				flag =true;
...
function empFileValition(){
	var emptfnForm = document.getElementById("emptfnForm");
	var emptfnfileName = emptfnForm.value;
	var emptfnfileExt = emptfnfileName.split(".");
	var emptfnfilefileExtLength = emptfnfileExt.length;
	var emptfnfileType = emptfnfileExt[emptfnfilefileExtLength-1];
	//console.log("emptfnForm"+emptfnfileName);
	
	var empsafFile = document.getElementById("empsafForm");
	var empsafFileName = empsafFile.value;
	var empsaffileExt = empsafFileName.split(".");
	var empsaffileExtLength = empsaffileExt.length;
	var empsaffileType = empsaffileExt[empsaffileExtLength-1];
	//console.log("empsaffileType"+empsaffileType);
	
	var empresumeFile = document.getElementById("empresumeForm");
	var empresumeFileName = empresumeFile.value;
	var empresumefileExt = empresumeFileName.split(".");
	var empresumefileExtLength = empresumefileExt.length
	var empresumefileType = empresumefileExt[empresumefileExtLength-1];
	
	
	console.log("empresumefileType"+empresumefileType);
	
	var empPayrollFile = document.getElementById("empPayrollForm");
	var empPayrollFileName = empPayrollFile.value;
	var empPayrollfileExt = empPayrollFileName.split(".");
	var empPayrollfileExtLength = empPayrollfileExt.length;
	var empPayrollfileType = empPayrollfileExt[empPayrollfileExtLength-1];
	console.log("empPayrollfileType"+empPayrollfileType);
	
	var flag=true;
	if(emptfnfileName != ""){
		
		if(emptfnfileType == 'pdf' || emptfnfileType == 'jpeg' || fileType == 'JPEG' || fileType == 'PDF'){
			$('#empTfnFormLabel').css('display','none');

		}
		else{
			$('#empTfnFormLabel').css('display','block');
			$("#empTfnFormLabel").css("color", "red");
			flag=false;
		}
	}
	if(empsafFileName != ""){
		
		if(empsaffileType == 'pdf' || empsaffileType == 'jpeg' || fileType == 'JPEG' || fileType == 'PDF'){
			$('#empsaFormLabel').css('display','none');

		}
		else{
			$('#empsaFormLabel').css('display','block');
			$("#empsaFormLabel").css("color", "red");	
			flag=false;
		}
	}
	if(empresumeFileName != ""){	
		
		if(empresumefileType == 'pdf' || empresumefileType == 'jpeg' || fileType == 'JPEG' || fileType == 'pdf'){
			$('#empResumeFormLabel').css('display','none');

		}
		else{
			$('#empResumeFormLabel').css('display','block');
			$("#empResumeFormLabel").css("color", "red");	
			flag=false;
		}
	
	}
	if(empPayrollFileName != ""){	
		
		if(empPayrollfileType == 'pdf' || empPayrollfileType == 'jpeg' || fileType == 'JPEG' || fileType == 'PDF'){
			$('#empPayrollFormLabel').css('display','none');

		}
		else{
			$('#empPayrollFormLabel').css('display','block');
			$("#empPayrollFormLabel").css("color", "red");	
			flag=false;
		}
	}
	return flag;
...
function drFileValidate(element){
	//element.value="";
	var drFileNameExt = element.value.split(".");
	var drfileExtLength = drFileNameExt.length;
	
	var drFileType = drFileNameExt[drfileExtLength-1];
	
	if(drFileType == 'pdf' || drFileType == 'jpeg' || drFileType == 'JPEG' || drFileType == 'PDF'){
...
function HoFileValidate(element){
	//element.value="";
	var HoFileNameExt = element.value.split(".");
	var HofileExtLength = HoFileNameExt.length;
	
	var HoFileType = HoFileNameExt[HofileExtLength-1];
	
	if(HoFileType == 'pdf' || HoFileType == 'jpeg' || HoFileType == 'JPEG' || HoFileType == 'PDF'){
...
function safePayoutFileValidate(element){
	//element.value="";
	var safePayoutFile = element.value.split(".");
	var safePayoutFileExtLength = safePayoutFile.length;
	var safePayoutFileType = safePayoutFile[safePayoutFileExtLength-1];
	if(safePayoutFileType == 'pdf' || safePayoutFileType == 'jpeg' || safePayoutFileType == 'JPEG' || safePayoutFileType == 'PDF'){
